...
export type DashboardTodo = { awaitingApproval: number; repliesToHandle: number; failedReminders: number };
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [summary, awaiting, replies, failed, activity] = await Promise.all([
  ...
  return {
    summary: parsed,
    todo: { awaitingApproval: awaiting.count ?? 0, repliesToHandle: replies.count ?? 0, failedReminders: failed.count ?? 0 },
    activity,
  };
}
