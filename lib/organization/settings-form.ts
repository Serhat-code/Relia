...
/** Réglages de l'organisation (§7) : identité et durée de conservation des données (§2.2). */

export const ORGANIZATION_FORM_FIELDS = ["name", "siren"] as const;
...
export type OrganizationSettings = { name: string; siren: string | null };
...
export function parseOrganizationForm(values: Partial<Record<(typeof ORGANIZATION_FORM_FIELDS)[number], string>>): OrganizationFormResult {
  const parsed = organizationSchema.safeParse({ name: values.name ?? "", siren: values.siren ?? "" });
  if (!parsed.success) return { ok: false, fieldErrors: firstFieldErrors(parsed.error) };
  return { ok: true, value: { name: parsed.data.name, siren: parsed.data.siren || null } };
}
